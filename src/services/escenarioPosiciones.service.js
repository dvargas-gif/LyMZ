...
export const escenarioPosicionesService = {
  ...
  /** Carga masiva (ej. desde Excel) dentro de una sala: un solo upsert con todas las filas, en tandas de a 1000. */
  async guardarLote(escenarioId, filas, usuarioId) {
    const ahora = new Date().toISOString();
    const filasDb = filas.map(f => ({ ...f, escenario_id: escenarioId, actualizado_por: usuarioId, actualizado_en: ahora }));
    for (let i = 0; i < filasDb.length; i += TAMANO_PAGINA) {
      const { error } = await supabase.from('escenario_posiciones').upsert(filasDb.slice(i, i + TAMANO_PAGINA));
      if (error) throw error;
    }
  },
};
