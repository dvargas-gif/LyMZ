 ...
 * pegados en la realidad (MZ02-03, MZ04-05, MZ06-07, ~0.645m entre sí --
 * ver ADR-010/011/012) vs. los que están solos (MZ01, MZ08, MZ09, MZ10,
 * ~3.5m de cualquier vecino). El espaciado visual usa esta agrupación
...
const GAP_ENTRE_GRUPOS = 22; // además del GAP normal -- separación extra entre grupos, no dentro de un par
...
const ETIQUETA_ALTO = 22; // franja arriba de las columnas verticales
const ETIQUETA_ANCHO = 54; // franja a la izquierda de cada fila horizontal
...
/** Y de la fila de cada pasillo horizontal, ya con el gap extra entre grupos aplicado -- una sola fuente de verdad para celdas y etiquetas. */
function calcularYPorPasillo() {
  const FILA_ALTO = CELDA_ALTO + GAP;
  const y = new Map();
  let cursor = ETIQUETA_ALTO;
...
export function calcularLayoutEsquematico() {
  ...
  ORDEN_HORIZONTAL.forEach(pasillo => {
    ...
    for (let c = 1; c <= columnas; c++) {
      celdas.push({
        pasillo, columna: c,
        x: xInicioHorizontal + (c - 1) * COL_ANCHO + corteAcumulado,
        y,
  ...
  ORDEN_HORIZONTAL.forEach(pasillo => {
    ...
    for (let c = 1; c <= columnas; c++) {
      if (cortesDeEstePasillo.includes(c)) {
        const xCelda = xInicioHorizontal + (c - 1) * COL_ANCHO + corteAcumulado;
...
export function calcularEtiquetas() {
  ...
  ORDEN_HORIZONTAL.forEach(pasillo => {
    etiquetas.push({ pasillo, x: xInicioHorizontal - ETIQUETA_ANCHO, y: yPorPasillo.get(pasillo), ancho: ETIQUETA_ANCHO, alto: CELDA_ALTO, vertical: false });
  });
