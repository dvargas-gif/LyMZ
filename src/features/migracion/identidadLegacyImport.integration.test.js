 ...
 *
 * Fixture: tests/fixtures/identidad_legacy_test.(xlsx|csv) -- 29 filas de
 * datos, mismo contenido en los dos formatos (generados desde el mismo
 * array). 20 válidas (fila 21 con espacios de sobra a propósito, confirma
 * que el trim funciona) + 9 rechazadas, una por cada categoría de error
 * real -- MZ duplicado y RCL duplicado ocupan 2 filas cada una porque una
 * duplicación no existe en una sola fila.
 */
...
const EXISTENTES = [{ mzPasillo: 'MZ09', mzColumna: 2, rclCodigo: 'RCL999-C999' }];
...
describe('import de identidad_legacy contra el fixture real (.xlsx)', () => {
  ...
  it('carga exactamente 20 filas válidas y rechaza 9', () => {
    expect(validas).toHaveLength(20);
    expect(rechazadas).toHaveLength(9);
  });

  it('acepta la fila con espacios al inicio/final tras el trim (fila 21)', () => {
  ...
  it('rechaza cada categoría de error con el motivo EXACTO prometido', () => {
    ...
    expect(motivoDe(22)).toBe('Celda vacía (falta MZ o RCL)'); // MZ vacío
    expect(motivoDe(23)).toBe('Celda vacía (falta MZ o RCL)'); // RCL vacío
    expect(motivoDe(24)).toBe('Formato de MZ inválido ("MZ1-C1") -- esperado MZ0X-C0YY');
    expect(motivoDe(25)).toBe('Formato de RCL inválido ("XYZ301-C001") -- esperado que empiece con RCL seguido de números');
    expect(motivoDe(26)).toBe('MZ duplicado dentro del archivo (MZ12-C001 aparece 2 veces)');
    expect(motivoDe(27)).toBe('MZ duplicado dentro del archivo (MZ12-C001 aparece 2 veces)');
    expect(motivoDe(28)).toBe('RCL duplicado dentro del archivo ("RCL304-C001" aparece 2 veces)');
    expect(motivoDe(29)).toBe('RCL duplicado dentro del archivo ("RCL304-C001" aparece 2 veces)');
    expect(motivoDe(30)).toBe('"RCL999-C999" ya está asignado a MZ09-C002 en la base -- no puede repetirse en MZ12-C005');
  });
...
describe('idempotencia -- importar el mismo fixture dos veces seguidas', () => {
  it('la segunda vuelta actualiza en vez de fallar, y el conteo final es igual al de la primera', () => {
    ...
    expect(primera.validas).toHaveLength(20);

    // Simula lo que guardarLote() habría dejado en la base tras la primera
    // vuelta (upsert real) -- las 20 filas válidas ya están, más lo que ya
    // había antes. Nunca se llama a Supabase acá (ver nota de la cabecera).
    const existentesTrasPrimeraVuelta = [
      ...EXISTENTES,
      ...primera.validas.map(f => ({ mzPasillo: f.mzPasillo, mzColumna: f.mzColumna, rclCodigo: f.rclCodigo })),
    ];
