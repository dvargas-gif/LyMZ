...
export function parsearFilaIdentidad(fila, raw) {
  ...
  if (!mzTexto || !rclTexto) {
    return { ...base, valido: false, motivo: 'Celda vacía (falta MZ o RCL)' };
  }
  ...
  if (!REGEX_RCL_PREFIJO.test(rclTexto.toUpperCase())) {
    return { ...base, valido: false, motivo: `Formato de RCL inválido ("${rclTexto}") -- esperado que empiece con RCL seguido de números` };
  }
  return {
    ...base, valido: true, motivo: '',
    mzPasillo: `MZ${matchMz[1]}`,
    mzColumna: parseInt(matchMz[2], 10),
    rclCodigo: rclTexto, // tal cual viene -- los sufijos (-001, -C001, -C002) NO se normalizan
  };
}
...
export function validarIdentidadLegacy(filasParsed, existentes = []) {
  const mzExistentePorRcl = new Map(existentes.map(e => [e.rclCodigo, claveMz(e.mzPasillo, e.mzColumna)]));
  ...
  for (const f of filasParsed) {
    ...
    conteoRcl.set(f.rclCodigo, (conteoRcl.get(f.rclCodigo) || 0) + 1);
  }
