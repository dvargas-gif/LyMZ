...
describe('parsearFilaIdentidad', () => {
  it('parsea una fila válida', () => {
    ...
    expect(fila).toEqual({
      ...
      valido: true, motivo: '', mzPasillo: 'MZ01', mzColumna: 1, rclCodigo: 'RCL121-C001',
    });
  });
  ...
  it('rechaza celda vacía (falta MZ o RCL)', () => {
    expect(parsearFilaIdentidad(2, { MZ: '', RCL: 'RCL121-C001' }).motivo).toMatch(/Celda vacía/);
    expect(parsearFilaIdentidad(2, { MZ: 'MZ01-C001', RCL: '' }).motivo).toMatch(/Celda vacía/);
  });
  ...
  it('rechaza formato de RCL inválido (no empieza con RCL+números)', () => {
