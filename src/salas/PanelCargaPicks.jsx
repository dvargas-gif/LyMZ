...
import { escenarioPicksService } from '../services/escenarioPicks.service.js';
import { reporteService } from '../services/reporte.service.js';
import { normalizarFilasPicks, parsearTextoPegado, calcularAnalisis } from '../services/analisisPicks.js';
        ...
        <tbody>
          {filas.map(f => (
            <tr key={f.articulo} style={{ borderTop: '1px solid #F0EEE5' }}>
              ...
              <td style={tdStyle}>{f.claseActual ? <span style={{ ...badgeStyle, background: colorDeClase(f.claseActual) }}>{f.claseActual}</span> : '—'}</td>
              <td style={{ ...tdStyle, fontFamily: 'monospace', fontSize: 11 }}>{f.pasilloActual ? `${f.pasilloActual}-C${String(f.columnaActual).padStart(3, '0')}` : '—'}</td>
