...
## Fase actual: **Canvas del mapa (react-konva) en rama separada — flag apagado, NO es Fase 2/bridge**

- 🟡 **`feat/mapa-canvas`** (rama pusheada a `origin`, commit `a5b1027`, 2026-07-08): reemplazo VISUAL completo del iframe legacy con `react-konva`, con paridad funcional casi total (ver auditoría abajo). **Vive en código real, en `src/features/mapa/canvas/`** — a diferencia del bridge de Fase 2 (que quedó solo documentado sin encontrarse en un checkout posterior), esto se puede verificar directo: `ls src/features/mapa/canvas/` en la rama `feat/mapa-canvas`. Detrás de `MAPA_CANVAS_HABILITADO = false` (`src/features/mapa/canvas/featureFlag.js`) — el iframe legacy sigue siendo el default en producción, sin excepción. **No fusionada a main. No activada. No es lo mismo que la Fase 2/Strangler Fig de abajo** (esa sigue en el mismo estado que quedó registrado, sin tocar).
...
## Canvas del mapa (`feat/mapa-canvas`) — qué se hizo y auditoría de paridad

Trabajo INDEPENDIENTE de las fases G0-G6/Strangler Fig de arriba — no reemplaza ni conecta con el bridge de Fase 2. Es un reemplazo VISUAL del iframe legacy (`react-konva`), construido en `src/features/mapa/canvas/` a lo largo de varias sesiones, con las mismas capacidades operativas (mover individual/cuerpo, bloquear, deshacer, buscar, exportar, selección de área en sala, añadir rack), restauradas después de una regresión funcional detectada por el usuario, más una serie de bugs reales encontrados en pruebas manuales:

- Guardado secuencial artículo-por-artículo al mover un cuerpo (N round-trips a Supabase) → corregido con `guardarLote()` (una sola llamada).
- Update visual pesimista (esperaba confirmación del server antes de pintar) → corregido a optimista con rollback si falla la persistencia.
- `dragend` de Konva burbujeando hasta el `Stage` y recentrando la cámara del mapa en (0,0) cada vez que se soltaba un rack.
- Redibujo del layer completo (~300 celdas) en cada frame de arrastre → cacheo por celda + handlers estables + `React.memo`.

**Auditoría de paridad pedida explícitamente por el usuario antes de considerar activar el flag alguna vez** (2026-07-08):

1. **Datos vs. geometría — confirmado con imports reales:** el Canvas usa el `WarehouseModel` real (`domain/crearWarehouseModel.js`) y las fórmulas de dominio reales (`domain/formulasOcupacion.js`) — mismos datos que el resto de la app. La **geometría física (x,y del DXF real, `GeometriaMezanine.js`/`geometriaMezanine.data.json`) NO se importa en ningún archivo de `src/features/mapa/canvas/`** (confirmado por grep) — el Canvas calcula su propia grilla esquemática (`posicionesEsquematicas.js`), documentada a propósito como "no 1:1 a los metros del DXF". Los NÚMEROS de esa grilla (columnas por pasillo) sí coinciden exactamente con `geometriaMezanine.data.json` (304/304 cuerpos, los 12 pasillos, ver abajo) — fueron transcritos a mano en su momento, no se importan dinámicamente.
2. **Los 12 pasillos reales (incluyendo MZ11/MZ12 verticales) están, con columnas reales exactas** — verificado pasillo por pasillo contra `geometriaMezanine.data.json`: MZ01=27, MZ02-07=36 c/u, MZ08=35, MZ09=4, MZ10=10, MZ11=5, MZ12=7, total 304/304. Coincidencia 100%. Único matiz: el DXF muestra a MZ08-MZ09 tan pegados entre sí (0.602m) como los pares ya confirmados (~0.645m), pero el Canvas los dibuja como filas sueltas — detalle puramente visual, no afecta datos.
3. **Gaps funcionales confirmados contra `INVENTARIO-LOGICA-MAPA.md` + lectura directa de `public/legacy/`:**
   - ❌ **Falta**: `limpiarSlot(key)` — botón para vaciar UN rack individual dentro de una sala (legacy `07-render.js:190`), distinto de "Limpiar área" (multi-selección), que sí existe en el Canvas.
   - ❌ **Falta**: terminal de cambios expandible (panel `#terminal` de legacy con el registro completo artículo/desde/hacia/hora) — el Canvas solo tiene el número en el badge de "Deshacer", no una vista de la lista.
   - ❌ **Falta**: color de alerta por artículo individual (>0.90 rojo, >0.60 ámbar, `07-render.js`) — el Canvas muestra el consumo en texto plano sin color. No es un hueco nuevo: `domain/configuracionOcupacion.js` ya tiene los números (`umbralArticulo`, ADR-005) pero nunca se implementó la función que los aplica, ni acá ni en ningún otro consumidor del dominio.
   - 🟡 **Funciona distinto**: el color por NIVEL usa la escala de RACK completo (1.0/0.85/0.4) en vez de la escala dedicada de nivel que tiene legacy (0.90/0.75, `07-render.js:198`).
   - 🟡 **Funciona distinto**: `intensidad()` (color interpolado por cantidad de artículos) reemplazado por color plano de clase + contador + barra de % — decisión de diseño ya tomada en una revisión estética anterior, no un olvido.
   - ✅ **Mejor que legacy**: `confirmar()` (mover 1 artículo) en legacy tiene un bug documentado que no envía `clase/tipo` (`INVENTARIO-LOGICA-MAPA.md` §35) — el Canvas sí los envía siempre.
   - Campo `grupo`: sigue sin trackearse (heredado del dominio desde G1b, ver "Decisión pendiente" en Fase 2 arriba — no es un hueco nuevo del Canvas).

**Estado real:** flag apagado (`MAPA_CANVAS_HABILITADO = false`), commiteado y pusheado a `origin/feat/mapa-canvas` (commit `a5b1027`), no fusionado a `main`, no activado en ningún ambiente. Los gaps de arriba quedan como lista de trabajo pendiente para la próxima vez que se considere activarlo — no bloquean nada más de este documento.
...
- `feat/mapa-canvas`: 4 gaps funcionales confirmados (ver sección de arriba) antes de que activar el flag sea una opción real — ninguno es bloqueante técnico, son trabajo pendiente.
...
- **2026-07-08 (sesión 13, rama `feat/mapa-canvas` — trabajo PARALELO a las fases G0-G6 de arriba, no continúa su numeración)** — Sesión larga de revisión funcional + visual + commit del Canvas del mapa (`react-konva`, construido en sesiones previas no registradas en este archivo por error — ver nota abajo). Se restauró paridad funcional completa (buscar/exportar/deshacer/mover/bloquear/selección de área/añadir rack), se agregó Restablecer Vista animado y toolbar moderna. Pruebas manuales del usuario encontraron 4 bugs reales, todos corregidos: guardado secuencial artículo-por-artículo al mover un cuerpo (ahora `guardarLote()` en una sola llamada), update visual pesimista (ahora optimista con rollback), `dragend` de Konva burbujeando al `Stage` y recentrando la cámara en (0,0) al soltar cualquier rack, y redibujo del layer completo en cada frame de arrastre (ahora cacheado por celda). También ajustes de paleta (fondo del canvas a gris, panel de detalle a blanco hueso) y limpieza de dead code (imports sin usar, inconsistencia de color en el minimapa). Se commiteó y pusheó a `origin/feat/mapa-canvas` (commit `a5b1027`) con el flag en `false`, sin fusionar a `main`. El usuario pidió, antes de considerar activar el flag alguna vez, una auditoría de paridad completa contra `INVENTARIO-LOGICA-MAPA.md` y la geometría real — resultado documentado en la sección "Canvas del mapa" arriba (datos reales confirmados por import, geometría propia pero con números exactos, 4 gaps funcionales confirmados). **Nota de higiene de proceso, pedida explícitamente por el usuario:** este archivo no se había actualizado durante las sesiones previas en las que se construyó el Canvas — se registra ahora para que no se repita lo que pasó con el bridge de Fase 2 (documentado como hecho pero no verificable en un checkout). Cualquier sesión futura que necesite el detalle completo de CÓMO se construyó el Canvas (no solo que existe) tiene que leer el código en `src/features/mapa/canvas/` directamente, no este archivo — la reconstrucción retroactiva completa de esas sesiones no se hizo acá.
