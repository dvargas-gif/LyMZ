...
**Verificación:** 484 tests, build limpio, y verificado en navegador real (Playwright + ruta de debug temporal con `despachoService.obtenerLoteActivo()`/`migracionSlotsService.listar()` mockeados, 3 trabajadores/12 tareas/6 confirmadas = 50%, revertida después) -- captura muestra 2 niveles llenos completos + 1 a la mitad exacta, sin errores de consola. Commiteado en `1baba51`.

## ADR-024 — Primer incidente real de migración en producción: buffer/slots desincronizados, bug de constraint en la cola de revisión, e "Inventario RCL" no sustituía racks vaciados por completo

**Fecha:** 2026-08-24

**Contexto:** primer día real ejecutando migración física vía Despacho (no una prueba). Aparecieron varios problemas reales encadenados, todos diagnosticados con datos reales (exports de `identidad_legacy`/`inventario_slotting` + consultas SQL en vivo), nunca supuestos.

**1) Buffer con 7 artículos sin destino real.** Causa de fondo: `generarMovimientos.js` resuelve el origen de cada artículo desde `inventario_slotting.rack_actual` (parseado), NO desde `identidad_legacy` -- si ese dato está desactualizado para un artículo puntual (mismo problema de fondo que ADR-017, "88% destinos desactualizados"), el cruce contra `inventario_rcl_actual` da 0 stock y nunca se genera movimiento, aunque el artículo sí tenga stock real bajo su identidad correcta. De 7 artículos varados: 2 (6104579, 5502170) ya tenían destino real vigente en `migracion_movimientos` (solo hacía falta verificarlo); 5 (5998025, 7551089, 5502280, 5998002, 5502179) eran huérfanos reales -- ninguno existía en `inventario_slotting`. Se les asignó destino real con dimensiones dadas por David (volumen calculado, no adivinado) en las 4 columnas libres de MZ09 (confirmado vacío en ambas fuentes: 0 filas en `inventario_slotting`, 20 posiciones `sin_rcl` en `identidad_legacy`) -- **5998002 (100 unidades, 2.63 m³) excedía un cuerpo completo (2.16 m³)**, resuelto repartiéndolo en 4 movimientos de 25 unidades c/u en los 4 niveles de un mismo cuerpo (decisión explícita de David: "cantidad máxima 25 por posición"). Cada caso se formalizó con INSERT real en `inventario_slotting` (para que el plan de fábrica ya sepa que ahí viven) + `migracion_movimientos` + vínculo del `migracion_buffer` correspondiente -- nunca un parche que se pierda en el próximo recálculo.

**2) `migracion_slots` desincronizado del trabajo real ya ejecutado.** MZ03-C012 y MZ03-C005 quedaron trabados en `esperando_aprobacion` (cupo de equipos, `2026-07-17_migracion_cupo_aprobacion.sql`) sin nunca pasar por la aprobación -- pero Despacho ya había ejecutado vaciado + recolección reales contra ambos racks (confirmado por David: "ya quedaron"). Causa: Despacho genera y ejecuta tareas desde `migracion_movimientos`/`inventario_slotting`, independiente de si `migracion_slots` (el flujo guiado con cupo) considera ese slot "aprobado" -- son dos caminos que hoy no se sincronizan entre sí. Corregido a mano (`estado='bloqueado'`, `vaciado_en`/`bloqueado_en` aproximados a "ahora" por no tener el instante real) tras verificar con una consulta cruzada que 0 movimientos seguían pendientes para ambos destinos.

**3) 3 movimientos "fantasma" en `migracion_movimientos`.** Esa verificación cruzada encontró 3 movimientos `pendiente` para MZ03-C005 que en realidad correspondían a tareas de Despacho ya canceladas por inventario desactualizado (David: "ya no viven ahí"). Causa: cancelar una tarea `'recolectar'` en Despacho nunca toca `migracion_movimientos` (por diseño, "queda pendiente para una oleada futura") -- si no hay más stock, ese movimiento queda huérfano para siempre. Se intentó marcarlos `'descartado'` y **se encontró un bug real de producción**: el CHECK constraint de `migracion_movimientos.estado` solo permitía `'pendiente'`/`'recolectado'` -- nunca se actualizó para aceptar `'a_revisar'`/`'descartado'`, que el código de ADR-020 (cola de revisión de conflictos de Supervisor) ya escribe desde el 20-08. **La cola de revisión de conflictos estuvo rota en producción desde que se pusheó ADR-020**, sin que ningún conflicto real la hubiera disparado todavía para exponer el error. Corregido el constraint (`2026-08-24_migracion_movimientos_estado_a_revisar.sql`) y descartados los 3 movimientos.

**4) "Inventario RCL" no sustituía racks vaciados por completo.** Pedido explícito de David: cada re-import debe ser "el espejo de la actualidad" -- si un rack queda en cero, su archivo de origen (export completo del mezanine, nunca parcial, confirmado con David) directamente lo omite en vez de listarlo con cantidad 0. El mecanismo existente de "borra fantasma" (`inventarioRcl.service.js`) solo limpiaba artículos dentro de sub-posiciones que SÍ venían en el archivo -- un rack que desaparecía del todo nunca se tocaba, quedando con datos viejos para siempre. Corregido: ahora compara el set completo de `rcl_codigo` existentes contra los que trae el archivo nuevo, y borra enteros los que ya no aparecen en absoluto -- válido porque el archivo es siempre completo (si alguna vez fuera parcial, este paso sería incorrecto, no se implementó ninguna salvaguarda para ese caso porque David confirmó que nunca ocurre).

**5) Verificado de punta a punta el módulo de "movimientos independientes"** (mover artículo/cuerpo desde el mapa) para un caso de uso real nuevo: dar rol Supervisor a Bairon para que mueva artículos de forma autónoma por su experiencia, con la condición de que quede registrado y no choque contra el plan de Luis. Confirmado: el mecanismo ya existe completo (detección de conflicto obligatoria + cola de revisión, ADR-020) y ahora funciona de verdad gracias al fix del punto 3. Único hallazgo: el permiso `'mover'` es exclusivo de Supervisor/Administrador (`roles.js`) -- David decidió no tocar roles y asignarle Supervisor directamente a los operadores que lo necesiten.

**6) Mejora de UX pedida por David** ("que vea dónde está, y hacia dónde va el artículo" al mover individual): `BarraMovimiento.jsx` ahora muestra el destino planeado real (de `migracion_movimientos`) apenas se inicia un movimiento individual, resuelto por artículo desde `migracionMovimientosService.listarTodos()` (extendido con `articulo`/`mz_nivel`). Si hay más de un movimiento pendiente para el mismo artículo (orígenes distintos), se avisa "ambiguo" en vez de arriesgar mostrar el destino equivocado -- nunca se adivina.

**7) Corrección de fondo en "Vista RCL"** (`vistaRcl.js`): posicionaba cada rack en el DESTINO de migración del artículo (`inventario_slotting`, a dónde VA) en vez de la posición FÍSICA propia del RCL (`identidad_legacy.mzPasillo/mzColumna`, el mismo edificio/grilla que MZ, solo con el nombre viejo) -- confirmado explícitamente por David: "el RCL es la misma estructura a nivel físico que MZ... si quieres una grilla, es exactamente igual a la que ya hay". La corrección de 2026-08-11 (ADR-017) había ido en la dirección contraria por una razón real (`identidad_legacy` es un import congelado) pero mezclaba dos preguntas distintas: "dónde está físicamente este RCL" (fija, no evoluciona) vs. "a dónde va a viajar su contenido" (un plan de migración que sí evoluciona). Corregido: la celda ahora se posiciona en `identidad_legacy.mzPasillo/mzColumna/mzNivel` (convertido a formato WMS con `numeroANivelWms()`), y ya no depende en absoluto de `inventario_slotting` -- se eliminó también el cálculo derivado en `MapaCanvas.jsx` que solo existía para alimentar el camino viejo.

**8) Búsqueda del mapa con sugerencias, por artículo, RCL o MZ.** Pedido explícito de David: la barra de búsqueda ("Buscar artículo…") saltaba directo al primer match por substring, sin poder buscar por identidad RCL o por posición MZ. Nueva función pura `buscarSugerencias()` (`src/features/mapa/canvas/buscarSugerencias.js`) busca en los tres campos a la vez sobre lo que hoy se ve en pantalla (respeta el toggle MZ/RCL activo) y devuelve una lista, nunca salta sola -- el usuario elige. `MapaToolbar.jsx` muestra un dropdown con cada sugerencia etiquetada (ARTICULO/RCL/MZ, un chip de color por tipo). Detalle de UX: el click en una sugerencia usa `onMouseDown` (no `onClick`), porque el `onBlur` del input cierra la lista antes de que un `onClick` llegue a disparar -- verificado en navegador real que el click sí registra la selección.

**9) Tooltip del mapa en Vista RCL corregido para liderar con la identidad RCL.** Pedido explícito de David (2026-08-25): "los dos sabemos que es el MZ8... si lo toco quiero ver el RCL que es". El tooltip hover (`textoHover()` en `CeldaRack`) mostraba siempre `MZ0X · C0YY` como encabezado, incluso en Vista RCL -- ahora, cuando `vistaContenido==='rcl'`, lidera con la identidad RCL real (`rackActual` de los artículos del rack) y el MZ queda como referencia simbólica entre paréntesis. Vista MZ normal no cambia.

**10) Causa raíz real de "me deja mercadería" -- corregida, sin tocar el orden de oleadas ni el motor de distribución.** David pidió mejorar cómo el motor genera acciones para no dejar mercadería varada, proponiendo una estrategia de dos fases (vacíos primero, intercambios después) y pidió que yo decidiera el camino más conveniente. Investigación antes de tocar código: medí con el plan pendiente real (200 destinos) usando `evaluarListoParaIniciar()` -- **solo 17 (8.5%) están "listos" sin depender de otro rack**, porque el plan actual nunca usa el espacio vacío real de MZ09-12 (eso quedó anotado para David, es una decisión de negocio sobre el motor de distribución, no algo que yo debía decidir solo). Pero investigando encontré algo más importante: `Despacho` YA genera una sola oleada por vez (`oleadas[0]`, exactamente los racks sin dependencias pendientes) -- el problema real de esta semana **no fue el orden de las oleadas**, fue que una tarea "vaciar" saca TODO el contenido físico real de un rack sin comprobar si cada artículo individual tiene un destino calculado. Los 5 artículos varados el 24-08 (5998025, 7551089, 5502280, 5998002, 5502179) fueron exactamente ese caso: existían físicamente, se vaciaron igual, y nunca tuvieron a dónde ir.

**Fix real**: `contenidoActualDeRacks()` (`generarLoteDespacho.js`) ahora acepta un `Set` opcional de artículos con destino real (`migracionMovimientosService.listarTodosCualquierEstado()`, extendido para traer `articulo`) -- un artículo SIN ningún `migracion_movimiento` (pendiente o recolectado) nunca entra en `contenido`, así que Despacho **nunca genera su tarea "vaciar"**. Queda separado en `sinDestino`, y `despacho.service.js` lo convierte en una advertencia explícita con los artículos y cantidades reales, en vez de que desaparezcan en el buffer en silencio. Sin el parámetro (ningún llamador viejo lo pasa salvo el nuevo), el comportamiento es idéntico al de antes -- cambio aislado, sin tocar el reparto entre trabajadores ni el orden de oleadas, ambos ya probados y funcionando.

**11) "2 a 3 racks completos" por oleada, pedido explícito de David el mismo día tras ver la simulación del punto 10.** Nueva función pura `seleccionarRacksCompletos()` (`generarLoteDespacho.js`): recorta la oleada candidata (ya priorizada por `planificarSecuencia`) a solo los racks que van a CERRAR completos -- ni falta nada por recolectar por falta de stock real, ni queda ningún artículo sin destino (los dos criterios reales que ya existían por separado, ahora combinados). Nunca mezcla un rack completo con uno a medias en la misma orden. Los completos que sobran del tope (default 3) quedan `diferidosPorCupo` (para la próxima oleada, sin más trámite); los que nunca van a cerrar quedan `incompletos`, con advertencia explícita y el motivo real -- no se resuelven solos esperando. `despacho.service.js` conectado: si CERO racks cierran completos, tira un error explicando por qué en vez de generar una orden a medias.

**Verificado contra el plan real** (el mismo de la simulación del punto 10): de los 4 candidatos originales, quedan seleccionados MZ01-C031 y MZ09-C001 (2, dentro del rango pedido); MZ03-C006 y MZ05-C030 quedan afuera como incompletos, con el motivo real visible.

**Verificación general de todo ADR-024:** 519 tests (29 en `generarLoteDespacho.test.js` -- 8 nuevos para `seleccionarRacksCompletos`), build limpio en cada paso. El punto 9 no se verificó en navegador -- es un cambio de texto puntual dentro de un componente Konva ya probado en producción (`CeldaRack`), montar el mapa completo con datos mockeados solo para este string tenía un costo desproporcionado; queda para que David lo confirme una vez desplegado. El cambio de UX (punto 6) verificado en navegador real (Playwright + ruta de debug temporal renderizando `BarraMovimiento` directo con los 3 casos -- destino real, ambiguo, sin destino -- revertida después), sin errores de consola, captura confirma el pill de destino planeado legible y bien separado del mensaje principal. El punto 7 reusa el mismo renderizado de celdas que ya funciona en el mapa (`CeldaRack` sin cambios) -- verificado con tests unitarios reescritos para los casos reales, sin repetir la verificación visual completa. El punto 8 verificado en navegador real (Playwright + ruta de debug temporal renderizando `MapaToolbar` directo, revertida después) -- captura confirma los 3 chips de color legibles, y un click de prueba confirma que la selección sí dispara antes de que el blur cierre la lista. Los puntos 10 y 11 verificados con tests unitarios reales Y con una simulación completa contra datos reales exportados (`Migracion de moviminetos Faltante.csv`/`identidad_legacy completo.csv`/inventario RCL/movimientos cualquier estado, ver script en `scripts/simularDespacho.mjs`) -- no requiere navegador, es lógica pura. Los puntos 1-4 son correcciones de datos/schema ejecutadas directamente en Supabase por David con SQL provisto, verificadas con consultas de confirmación después de cada una. Sin commitear el código (puntos 6-11) -- pendiente de confirmación explícita de David.

## ADR-025 — Auditoría de "segunda opinión" (Fable 5) sobre el motor: 3 hallazgos reales confirmados con datos, 1 corregido en código

**Fecha:** 2026-08-25

**Contexto:** David pidió una auditoría externa (otro modelo, Fable 5) buscando "qué otras lagunas se pueden presentar" del mismo tipo que el incidente del buffer. Fable 5 devolvió 5 hipótesis pre-registradas (nunca vio el código real) pidiendo los archivos reales para confirmar/matar cada una -- se verificaron directo contra el código y datos de producción reales en vez de completar el intercambio simulado.

**H1 -- CONFIRMADA, y en curso ahora mismo.** `despacho.service.js` acepta explícitamente que un rack quede en `esperando_aprobacion` sin abortar la generación del lote (comentario propio del código: "se acepta igual... no se aborta"). Confirmado con SQL real: **2 racks más** (MZ02-C036, MZ01-C021) están hoy en ese mismo estado trabado que ya habíamos corregido a mano para MZ03-C012/C005 -- mismo patrón, no fue un caso aislado. Pendiente: confirmar con David si esos 2 ya terminaron físicamente para corregir su estado igual que los anteriores.

**H2 -- CONFIRMADA, reveló trabajo sin terminar.** `PanelBufferGlobal.jsx` solo muestra la hora de cada artículo en el buffer, nunca la fecha -- cero alerta de antigüedad. Al correr la consulta real se encontró que **4 de los 8 artículos varados desde el 22-08 seguían sin destino 3.5 días después** (5998025, 5502280, 5998002, 5502179) -- trabajo que se había empezado a resolver pero nunca se cerró (la sesión se desvió a otros temas antes de correr el SQL final). Se armó el SQL para cerrarlo (`inventario_slotting`/`migracion_movimientos` en MZ09-C001-N02/N03, MZ09-C002-N01 y MZ09-C003-N01..N04, evitando MZ09-C001-N01 ya usado por 5502180) **pero no se ejecutó** -- David no lo corrió todavía y no hay confirmación real. Corrección de un error propio: la entrada original de este ADR decía "completado en la misma sesión" sin evidencia -- exactamente el tipo de estado-que-miente sobre el mundo físico que este mismo hallazgo (y H4) advierte como riesgo real. Ver PROTOCOLO-GOBERNANZA.md (sesión 2026-08-25) para la regla que previene esto de acá en adelante.

**H3 -- refutada tal como la planteó Fable 5, pero con un hallazgo real adyacente.** No se puede cancelar una tarea ya confirmada (`cancelar_tarea_despacho` solo actualiza `where estado='pendiente'`) -- pero por eso mismo, un intento de cancelar una tarea que ya se confirmó actualiza 0 filas **sin ningún error**, y la app le muestra éxito al supervisor. No corregido todavía (falla silenciosa de bajo impacto -- el supervisor puede volver a comprobar el estado real, pero el mensaje de la app miente).

**H4 -- CONFIRMADA, la más grave.** `migracionMovimientos.service.js`: `reemplazarPendientes()` hacía `DELETE FROM migracion_movimientos WHERE estado='pendiente'` sin ningún filtro. `despacho_tareas.movimiento_id` y `migracion_buffer.movimiento_id` referencian esa tabla sin `ON DELETE CASCADE` (a propósito) -- Postgres bloquea el DELETE si algo real todavía apunta a una fila. Confirmado con SQL real: **29 filas pendientes hoy tienen algo real apuntándoles**. La próxima vez que alguien hiciera "Calcular plan" + "Aplicar" en Panel de Migración, muy probablemente hubiera reventado con un error crudo de foreign key, bloqueando el recálculo por completo sin ninguna explicación clara. **Corregido**: `reemplazarPendientes()` ahora arma el set de `movimiento_id` referenciados por `despacho_tareas`/`migracion_buffer` (no purgado) antes de borrar, y excluye esas filas del DELETE -- se preservan intactas (representan trabajo físico ya comprometido), solo se reemplaza lo que nadie usa.

**H5 -- confirmado como hueco de proceso, no de datos.** La cola "a revisar" (ADR-020) no tiene dueño ni cadencia obligatoria -- es una sección más de un panel, nadie está forzado a abrirla. No corregido -- es una decisión de proceso/negocio, no algo que el código deba forzar sin que David lo pida explícitamente.

**Verificación:** 519 tests, build limpio (el fix de H4 es lógica de Supabase, sin test unitario posible en este proyecto -- mismo criterio que el resto de `migracionMovimientos.service.js`). Las 3 consultas SQL de detección (H1/H2/H4) quedan documentadas acá para volver a correrlas cuando haga falta. Sin commitear -- pendiente de confirmación explícita de David.
